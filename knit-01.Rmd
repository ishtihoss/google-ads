...
aug25toNov22_demographic %>% group_by(age) %>% summarise(sImpr=sum(impr),sClicks=sum(clicks),sConv=sum(conversions),sCost=sum(cost))
```
## Grouping Audience by Gender

```{r audience by gender, echo=FALSE}
aug25toNov22_demographic %>% group_by(gender) %>% summarise(sImpr=sum(impr),sClicks=sum(clicks),sConv=sum(conversions),sCost=sum(cost))
```

## Grouping Audience by Income Group

```{r, audience by household income, echo=FALSE}
aug25toNov22_demographic %>% group_by(household_income) %>% summarise(sImpr=sum(impr),sClicks=sum(clicks),sConv=sum(conversions),sCost=sum(cost))
```
